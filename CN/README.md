...
- 병목 링크의 처리율이 종단 간 처리율
    - 처리율은 min(R1, R2)
- 예) 10개의 클라이언트가 10개의 서버로부터 다운로드
    - 공통 링크(backbone)는 10개의 다운로드에 공평하게 전송 속도를 나눈다고 가정
    - 종단 간 처리율은 min(R1,R2,R/10)
...
- 계층(Layers): 각 계층은 서비스를 구현
    - 자신의 계층 내부의 동작을 서비스로 구현
    - 아래 계층에서 제공되는 서비스에 의존
- 계층화를 통해 크고 복잡한 시스템을 관리
- 명확한 구조화를 통해 복잡한 시스템 요소를 구분하고, 요소 간의 관계 설정이 가능
- 시스템의 유지보수와 변경이 용이
    - 한 계층의 서비스 구현이 변경되어도 시스템의 나머지 부분에 영향 없음
...
- 애플리케이션(application)
    ...
    - FTP, SMTP, HTTP
- 트랜스포트(transport)
    - 클라이언트와 서버 간에 애플리케이션 계층 메시지를 전달하는 서비스 제공
    - 프로세스-프로세스 데이터 전송
    - TCP, UDP
- 네트워크(network)
    - 한 호스트에서 다른 호스트로 데이터그램을 라우팅한다.
    - IP
- 링크(link)
...
- 세션(session)
    - 데이터 교환의 경계와 동기화
    - 데이터의 체킹 포인트와 회복 방법을 세우는 수단

#### 캡슐화

- 캡슐화: 송신 데이터에 필요한 정보를 붙여서 다음 계층에 보내는 것
- 비캡슐화: 캡슐화의 반대 개념으로, 헤더를 제거하는 것

패킷 = 헤더(header) 필드 + 페이로드(payload) 필드

- 응용 프로그램 계층
    - 메시지: 응용 프로그램이 전송 계층으로 전송하기를 원하고 전달되는 데이터
- 전송 계층 
    - 세그먼트: 전송 계층에 의해 에너지가 공급되고 전송 계층 헤더로 애플리케이션 계층 메시지를 캡슐화한다.
- 네트워크 계층 
    - 데이터그램: 네트워크 계층 헤더로 전송 계층 세그먼트를 캡슐화한다.
- 링크 계층 
    - 프레임: 링크 계층 헤더로 네트워크 계층 데이터그램을 캡슐화한다.

## 2. 애플리케이션 계층

### 2.1 네트워크 애플리케이션의 원리

#### 2.1.1 네트워크 애플리케이션 개발

- 애플리케이션 프로그램
    - 서로 다른 종단 시스템에서 동작
    - 네트워크 상에서 통신
    - 예) 웹 서버 프로그램과 통신하는 웹 브라우저 프로그램
- 네트워크 코어 장비는 사용자 애플리케이션을 실행하지 않으므로 종단 시스템의 애플리케이션을 빠르게 개발하고 전파가 가능

#### 2.1.2 네트워크 애플리케이션 구조
...
- 프로세스는 소켓을 통해 네트워크로 메시지를 송수신
    - 소켓은 호스트의 애플리케이션 계층과 트랜스포트 계층 간의 인터페이스
    - 프로세스는 집, 소켓은 출입구에 비유
        - 송신 프로세스는 출입구 바깥 네트워크로 메시지를 밀어냄
    - 소켓은 애플리케이션과 네트워크 사이의 API(Application Programming Interface)

#### 프로세스의 주소

- 메시지 수신을 위해 프로세스를 구분하는 식별자가 있어야 함
- 프로세스가 수행되는 호스트의 IP 주소로 프로세스들을 식별할 수 있는가?
    - No, 호스트에서 많은 프로세스들이 동시에 수행될 수 있음
- 호스트 상의 프로세스는 IP 주소와 포트 번호로 구분

#### 애플리케이션 계층 프로토콜

- 애플리케이션 계층 프로토콜은 다음과 같이 정의
    - 교환되는 메시지 타입
        - 예) 응답 메시지, 요청 메시지
    - 여러 메시지 타입의 문법
        - 예) 메시지 내부의 필드와 필드 간의 구별 방법
    - 메시지 의미
        - 필드에 있는 정보의 의미
